...
def _candle(ts: int, o: float, h: float, low: float, c: float) -> dict:
    return {"ts": ts, "open": o, "high": h, "low": low, "close": c, "volume": 100.0}
...
async def test_trade_verify_win_path() -> None:
    ...
    result = await service.verify("BTC", "1h", datetime(2025, 1, 1, tzinfo=UTC), 100, 95, [105], mode="ambiguous")
...
async def test_trade_verify_same_candle_conservative() -> None:
    ...
    candles = [_candle(start, 100, 106, 94, 101)]
    service = TradeVerifyService(DummyOHLCV(candles))
    result = await service.verify("BTC", "1h", datetime(2025, 1, 1, tzinfo=UTC), 100, 95, [105], mode="conservative")
