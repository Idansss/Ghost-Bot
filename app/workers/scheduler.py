...
class WorkerScheduler:
    ...
    async def _process_alerts(self) -> None:
        async with self.hub.cache.distributed_lock("task:alerts", ttl=self.settings.alert_check_interval_sec + 10) as acquired:
            if not acquired:
                return
            try:
    ...
    async def _refresh_news_cache(self) -> None:
        async with self.hub.cache.distributed_lock("task:news", ttl=14 * 60) as acquired:
            if not acquired:
                return
            try:
    ...
    async def _process_giveaways(self) -> None:
        async with self.hub.cache.distributed_lock("task:giveaways", ttl=15) as acquired:
            if not acquired:
                return
            try:
    ...
    async def _refresh_scan_universe(self) -> None:
        async with self.hub.cache.distributed_lock("task:scan_universe", ttl=29 * 60) as acquired:
            if not acquired:
                return
            try:
    ...
    async def _refresh_scan_indicators(self) -> None:
        async with self.hub.cache.distributed_lock("task:scan_indicators", ttl=4 * 60) as acquired:
            if not acquired:
                return
            try:
    ...
    async def _process_market_broadcasts(self) -> None:
        async with self.hub.cache.distributed_lock("task:broadcasts", ttl=max(4, int(self.settings.broadcast_interval_minutes) - 1) * 60) as acquired:
            if not acquired:
                return
            try:
    ...
    async def _process_scheduled_reports(self) -> None:
        async with self.hub.cache.distributed_lock("task:scheduled_reports", ttl=55) as acquired:
            if not acquired:
                return
            try:
