...
async def _handle_routed_intent(message: Message, settings: dict, route: dict) -> bool:
    ...
    if intent == "rsi_scan":
        ...
        rsi_length = max(2, min(_as_int(params.get("rsi_length"), 14), 200))
    ...
    if intent == "alert_create":
        ...
        await message.answer(
            f"Alert created: #{alert.id}\nCondition: {alert.symbol} {alert.condition} {alert.target_price}\nI trigger once and avoid spam."
        )
        return True
    ...
    if intent == "alert_list":
        ...
        if not alerts:
            ...
            lines = [f"#{a.id} {a.symbol} {a.condition} {a.target_price} [{a.status}]" for a in alerts]
...
async def alert_cmd(message: Message) -> None:
    ...
    if text.startswith("/alert list"):
        ...
        if not alerts:
            await message.answer("No active alerts.")
            return
        rows = [f"#{a.id} {a.symbol} {a.condition} {a.target_price} [{a.status}]" for a in alerts]
        await message.answer("Active alerts:\n" + "\n".join(rows))
        return
...
async def refresh_callback(callback: CallbackQuery) -> None:
    hub = _require_hub()
    symbol = (callback.data or "").split(":", 1)[1]
    settings = await hub.user_service.get_settings(callback.message.chat.id)
    payload = await hub.analysis_service.analyze(
        symbol,
    ...
    await hub.cache.set_json(f"last_analysis:{callback.message.chat.id}:{symbol}", payload, ttl=1800)
...
async def save_wallet_callback(callback: CallbackQuery) -> None:
    ...
    await hub.wallet_service.scan(chain, address, chat_id=callback.message.chat.id, save=True)
...
async def route_text(message: Message) -> None:
    ...
    if message.chat.type in ("group", "supergroup"):
        ...
        if not text:
            await message.answer("Say a ticker + direction, e.g. `SOL long`.")
            return
    ...
    if pending_alert_symbol:
        ...
        if price_match:
            ...
            await message.answer(f"Alert created: #{alert.id} {pending_alert_symbol} cross {target}")
            return
    ...
    if parsed.requires_followup:
        ...
        if parsed.intent == Intent.ANALYSIS and not parsed.entities.get("symbol"):
            ...
            return
        await message.answer(parsed.followup_question or "Need one extra detail.")
        return
